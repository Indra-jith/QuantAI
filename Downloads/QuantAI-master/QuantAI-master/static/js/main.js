document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            // Try to get symbol from hidden input or company name input
            ...
            // If hidden symbol is empty, check if user entered a symbol manually
            if (!symbol && companyInput.value.trim()) {
                const manualInput = companyInput.value.trim().toUpperCase();
                // Basic validation: 1-5 letters
                if (/^[A-Z]{1,5}$/.test(manualInput)) {
                    symbol = manualInput;
            ...
            if (!symbol) {
                alert('Please select a stock from the suggestions or enter a valid symbol');
                return;
            }
...
// analyzeStock function removed in favor of event listener
