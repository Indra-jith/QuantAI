...
class StockDataProcessor:
    ...
    def get_stock_data(self, symbol: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            if not time_series:
                logger.error(f"Full API response for {symbol}: {json.dumps(data)}")
    ...
    def _calculate_indicators(self, df: pd.DataFrame, symbol: str, period: str = 'training') -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                # Additional momentum and price position indicators
                price_window = 20 if period == 'training' else 10
                df['Price_Position'] = (df['close'] - df['low'].rolling(price_window, min_periods=1).min()) / \
                                       (df['high'].rolling(price_window, min_periods=1).max() - df['low'].rolling(price_window, min_periods=1).min() + 1e-10)
                df['ROC'] = df['close'].pct_change(periods=10 if period == 'training' else 3)  # Rate of change - smaller for test
                df['ATR'] = (df['high'] - df['low']).rolling(window=14 if period == 'training' else 5, min_periods=1).mean()  # Average True Range
                ...
                required_indicators = [
                    ...
                    'Volume_MA5', 'Price_Position', 'ROC', 'ATR'
                ]
...
class DQN(nn.Module):
    """Enhanced DQN architecture with layer normalization and dropout."""
...
class StockTradingEnvironment:
    def __init__(self, data: pd.DataFrame, initial_balance: float = 10000):
        ...
        self.reset()
        self.state_size = 15  # Updated to include new features
        self.action_size = 3  # hold, buy, sell
        self.transaction_fee = 0.0005  # 0.05% transaction fee
    ...
    def reset(self):
        ...
        self.total_profit = 0
        self.last_trade_step = 0  # Track last action step
        self.trading_history = []  # Reset trading history
        self._portfolio_value = self.initial_balance
        return self._get_state()

    def step(self, action: int) -> Tuple[np.ndarray, float, bool]:
        """Execute one step in the environment with improved reward function."""
        ...
        # Enhanced reward function with aggressive signals
        portfolio_change = (new_portfolio_value - old_portfolio_value) / old_portfolio_value
        reward = portfolio_change * 1000  # Much larger scale for visibility
        
        # Strong bonuses for taking actions
        if action == 1 and self.shares_held > 0:  # Successful buy
            reward += 2.0  # Significant bonus for buying
        elif action == 2 and self.shares_held == 0:  # Successful sell
            profit_ratio = (self.total_profit / self.initial_balance) if self.initial_balance > 0 else 0
            reward += profit_ratio * 50  # Large bonus for profitable trades
            if profit_ratio > 0:
                reward += 5.0  # Extra bonus for any profit
        
        # Much stronger penalty for doing nothing
        holding_time = self.current_step - self.last_trade_step
        if action == 0:  # Holding
            if self.shares_held > 0 and holding_time > 10:  # Holding stock too long
                reward -= 0.5
            elif self.shares_held == 0 and holding_time > 5:  # Not buying for too long
                reward -= 0.3
    ...
    def _get_state(self):
        ...
        try:
            state = np.array([
                ...
                current_data['Volume_MA5'] / (current_data['volume'] + 1e-10) - 1,  # Volume trend
                current_data['close'] / current_data['open'] - 1,  # Daily return
                current_data['Price_Position'],  # Price position in range
                current_data['ROC'],  # Rate of change
                current_data['ATR'] / current_data['close']  # Normalized ATR
            ], dtype=np.float32)
...
class TradingAgent:
    ...
    def train(self, env, episodes=100, batch_size=64):
...
def initialize_trading_environment(symbol: str, initial_balance: float = 10000) -> Tuple[TradingAgent, StockTradingEnvironment, StockTradingEnvironment]:
